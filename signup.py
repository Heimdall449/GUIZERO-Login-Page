...
def signup(): # Handle the signup process when the user clicks the signup button
    ...
    if password != confirm: # If passwords do not match, display error text
        error_text.set("Passwords do not match")
        if error_label:
            error_label.config(fg="red")
        return

    password_error = is_password_valid(username, password) # if password doesn't fufill all requirements, display red error text.
    if password_error:
    ...
    save_user(username, password) # If passwords fufill all requirements, display green success text, opens window that states successful login.
